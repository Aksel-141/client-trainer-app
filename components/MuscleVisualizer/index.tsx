...
export default function MuscleVisualizer({ muscleList }: MuscleVisualizerProps) {
  console.log("Список м'язів для візуалізатора:", muscleList);
  ...
  function ChangeMuscleColor() {
    ...
    const normalizedSet = new Set((muscleList || []).map((m) => normalize(m)));
    ...
    groups.forEach((item) => {
      ...
      if (normalizedSet.has(normId)) {
        item.classList.add("muscle-highlight");
        matchedIds.push(id);
      } else {
        item.classList.remove("muscle-highlight");
      }
    });

    console.log("[DEBUG] MuscleVisualizer normalized input:", Array.from(normalizedSet));
    console.log("[DEBUG] SVG group ids:", allIds.slice(0, 50));
    console.log("[DEBUG] Matched SVG ids:", matchedIds);
  }
  ...
  return (
    <div style={{ width: "100%", display: "flex" }}>
      {/* --Front view-- */}
      <svg viewBox="0 0 660.46 1206.46" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-80 col-span-2">
        <defs>
          <radialGradient id="jointradial" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
            <stop
              offset="0%"
              //   style="stop-color: rgb(254, 91, 127); stop-opacity: 1;"
              //   style={{ stopColor: "rgb(254, 91, 127)" }}
            ></stop>
            <stop
              offset="100%"
              //   style="stop-color: rgb(231, 236, 239); stop-opacity: 1;"
              //   style={{ stopColor: "rgb(231, 236, 239" }}s
            ></stop>
          </radialGradient>
        </defs>
